...
from dataUtils.basic.config_file import JSON_PATH_FILE
...
class JsonFileHandler:
    ...
    """
    def __init__(self, json_path_file=JSON_PATH_FILE):
    ...
    def read_json_into_mapping_str(self, json_file_name: str):
        ...
        try:
            for i in data["mapping"]:
                file_list.append('"' + i['source'] + '" AS ' + i['target'])
        except Exception as e:
            raise AirflowException("Json object must be named 'mapping', each column must have a 'source and 'target'")
