/* 
  The query is done to convert from bronze data into silver data
     ...
     Done by: ThuLHA2
     Last update when: 2024-03-22 09:34:00.0000000
     Update content: Edit primary key format and query template
*/

TRUNCATE TABLE IF EXISTS PURCHASE.PURCHASE_SILVER.STOCKITEM; -- This statement clears out any existing data in the silver table if it exists.

-- This part begins the insertion of data into the silver table.
INSERT INTO PURCHASE.PURCHASE_SILVER.STOCKITEM (
    StockItemKey,
    WWIStockItemID,
    StockItem,
    Color,
...
FROM PURCHASE.PURCHASE_BRONZE.STOCKITEMS as S 
    left join PURCHASE.PURCHASE_BRONZE.COLORS as C 
...
FROM a 
    left join PURCHASE.PURCHASE_BRONZE.packagetypes as P1 
...
FROM b
    left join PURCHASE.PURCHASE_BRONZE.packagetypes as P2 
