...
Module: MOVEMENT
Contributed Bronze Table: MOVEMENT.MOVEMENT_BRONZE.CUSTOMERS,MOVEMENT.MOVEMENT_BRONZE,
MOVEMENT.MOVEMENT_BRONZE.BUYINGGROUPS, .CUSTOMERCATEGORIES,MOVEMENT.MOVEMENT_BRONZE.PEOPLE, 
Silver table: MOVEMENT.MOVEMENT_SILVER.CUSTOMER
Done by: PHUBT
Last update when: 2024-03-25 17:02:00.0000000
Update content: fix Case When, ValidTO, ValidFrom
*/
TRUNCATE MOVEMENT.MOVEMENT_SILVER.CUSTOMER;
-- Insert updated customer data into the MOVEMENT.MOVEMENT_SILVER.CUSTOMER table.
INSERT INTO MOVEMENT.MOVEMENT_SILVER.CUSTOMER (
    CUSTOMERKEY,
    WWICUSTOMERID,
    CUSTOMER,
    BILLTOCUSTOMER,
                                        ...
                                        SELECT 
                                            ...
                                            FROM(
                                                SELECT
                                                    ...
                                                    FROM
                                                        MOVEMENT.MOVEMENT_BRONZE.CUSTOMERS T1
                                                        LEFT JOIN MOVEMENT.MOVEMENT_BRONZE.CUSTOMERCATEGORIES T2 ON T1.CUSTOMERCATEGORYID = T2.CUSTOMERCATEGORYID
                                                        AND T1.VALIDFROM < T2.VALIDTO AND T1.VALIDTO > T2.VALIDFROM) AS V1
                                            LEFT JOIN MOVEMENT.MOVEMENT_BRONZE.BUYINGGROUPS T3 ON V1.BUYINGGROUPID = T3.BUYINGGROUPID
                                            AND V1.VALIDFROM < T3.VALIDTO AND V1.VALIDTO > T3.VALIDFROM) AS V2
                                        INNER JOIN MOVEMENT.MOVEMENT_BRONZE.CUSTOMERS T4 ON V2.BILLTOCUSTOMERID = T4.CUSTOMERID
                                        AND V2.VALIDFROM < T4.VALIDTO AND V2.VALIDTO > T4.VALIDFROM) AS V3
                        LEFT JOIN MOVEMENT.MOVEMENT_BRONZE.PEOPLE T5 ON V3.PRIMARYCONTACTPERSONID = T5.PERSONID
